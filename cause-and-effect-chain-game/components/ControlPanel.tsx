...
const InfoPill: React.FC<{ elements: ElementsMap; label: string; values: string[]; colorClass: string; bmLabel: string }> = ({ elements, label, values, colorClass, bmLabel }) => (
    <div>
        <SimpleTooltip bmText={bmLabel} enabled={true}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <div className="grid grid-cols-1 gap-y-2 mb-3">
            <InfoPill elements={elements} label="Causes" values={Array.from(selectedCauses)} colorClass="bg-rose-100 text-rose-800" bmLabel="Sebab" />
            <InfoPill elements={elements} label="Effect" values={selectedEffect ? [selectedEffect] : []} colorClass="bg-sky-100 text-sky-800" bmLabel="Akibat" />
          </div>
        ...
        <SimpleTooltip bmText="Buat Hubungan" enabled={true}>
          <button
